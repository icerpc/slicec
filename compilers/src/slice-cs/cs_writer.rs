...
impl Visitor for CsWriter {
    fn visit_file_start(&mut self, _: &SliceFile, _: &SliceAst) {
        self.file_buffer.write("//Start of file\n".as_bytes()).unwrap();
    }

    fn visit_file_end(&mut self, _: &SliceFile, _: &SliceAst) {
        self.file_buffer.write("//End of file\n".as_bytes()).unwrap();
    }

    fn visit_module_start(&mut self, module_def: &Module, _: usize, _: &SliceAst) {
    ...
    fn visit_struct_start(&mut self, struct_def: &Struct, _: usize, _: &SliceAst) {
    ...
    fn visit_interface_start(&mut self, interface_def: &Interface, _: usize, _: &SliceAst) {
    ...
    fn visit_data_member(&mut self, data_member: &DataMember, _: usize, _: &SliceAst) {
