...
fn return_tuple_must_contain_two_or_more_elements() {
    ...
    // Assert
    let expected = Error::new(LogicKind::ReturnTuplesMustContainAtLeastTwoElements, None);
...
mod streams {
    ...
    fn operation_can_have_at_most_one_streamed_parameter() {
        ...
        // Assert
        let expected = Error::new(LogicKind::StreamedMembersMustBeLast("s2".to_owned()), None); // TODO: This is checking the wrong error
        assert_errors!(diagnostic_reporter, [&expected]);
    }
    ...
    fn stream_parameter_must_be_last() {
        ...
        // Act
        let expected = Error::new(LogicKind::StreamedMembersMustBeLast("s".to_owned()), None);
