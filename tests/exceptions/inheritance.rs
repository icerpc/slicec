...
fn does_not_support_multiple_inheritance() {
    ...
    // Assert
    let expected = Error::new(LogicKind::CanOnlyInheritFromSingleBase("exception".to_string()), None);
...
fn data_member_shadowing_is_disallowed() {
    ...
    // Assert
    let expected = Error::new_with_notes(LogicKind::Shadows("i".to_owned()), None, vec![Note::new(
