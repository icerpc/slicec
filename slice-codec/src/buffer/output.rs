...
impl<'a> SliceOutputTarget<'a> {
    fn ensure_target_has_at_least(&self, requested: usize) -> Result<()> {
        let remaining = self.target.len() - self.pos;
...
impl<'a> OutputTarget<'a> for SliceOutputTarget<'a> {
    fn write_bytes_exact(&mut self, bytes: &[u8]) -> Result<()> {
        self.ensure_target_has_at_least(bytes.len())?;

        // SAFETY: the above function call guarantees there's enough space in `self.target` to write `bytes`,
        // and we know the slices cannot overlap because the mutable borrow of `self` guarantees exclusive access.
        unsafe {
            let end = self.pos + bytes.len();
            debug_assert!(self.target.get_mut(self.pos..end).is_some());
            let target_slice = self.target.get_unchecked_mut(self.pos..end);
    ...
    fn write_bytes_into_reserved_exact(&mut self, reservation: &mut Reservation, bytes: &[u8]) -> Result<()> {
        ...
        let Some(reserved_slice) = self.target.get_mut(reservation.range()) else {
            let error = ErrorKind::InvalidReservation {
                buffer_len: self.target.len(),
    ...
    fn reserve_space(&mut self, count: usize) -> Result<Reservation> {
        self.ensure_target_has_at_least(count)?;
...
impl<'a> OutputTarget<'a> for VecOutputTarget<'a> {
    fn write_bytes_exact(&mut self, bytes: &[u8]) -> Result<()> {
        ...
        // SAFETY: the above function call guarantees there's enough spare capacity in `self.target` to write `bytes`,
        // and we know the slice cannot overlap because the mutable borrow of `self` guarantees exclusive access.
        unsafe {
            debug_assert!(self.target.spare_capacity_mut().get_mut(..count).is_some());
            let target_slice = self.target.spare_capacity_mut().get_unchecked_mut(..count);
            ...
            let old_length = self.target.len();
            self.target.set_len(old_length + count);
            Ok(())
        }
    }
    ...
    fn reserve_space(&mut self, count: usize) -> Result<Reservation> {
        ...
        // SAFETY: the above function call guarantees there's enough spare capacity in `self.target` for `count` bytes,
        // and `0x00` is a valid memory representation for a `u8`.
        unsafe {
            let pos = self.target.len();
            let end = pos + count;
            debug_assert!(self.target.spare_capacity_mut().get(..count).is_some());
            let target_offset = self.target.as_mut_ptr().add(pos);
...
mod tests {
    ...
    mod vec_output_target {
        ...
        fn write_bytes_exact() {
            ...
            assert_eq!(output.target.len(), 0);
            assert_eq!(output.target.capacity(), 8);

            output.write_bytes_exact(&[8, 5, 3, 100]).unwrap();
            assert_eq!(output.target.len(), 4);
            assert_eq!(output.target.capacity(), 8);
            assert_eq!(output.target.as_slice(), &[8, 5, 3, 100]);

            output.write_bytes_exact(&[1, 2, 3, 4, 5, 6]).unwrap();
            let new_capacity = output.target.capacity();
            assert_eq!(output.target.len(), 10);
            assert!(new_capacity >= 10); // We don't know how much it'll grow.
            assert_eq!(output.target.as_slice(), &[8, 5, 3, 100, 1, 2, 3, 4, 5, 6]);

            output.write_bytes_exact(&[]).unwrap();
            assert_eq!(output.target.len(), 10);
            assert_eq!(output.target.capacity(), new_capacity);
            assert_eq!(output.target.as_slice(), &[8, 5, 3, 100, 1, 2, 3, 4, 5, 6]);
        }
        ...
        fn write_bytes_exact_into_reserved() {
            ...
            assert_eq!(output.target.len(), 6);
            assert_eq!(output.target.capacity(), 8);
            assert_eq!(output.target.as_slice(), &[0, 0, 0, 0, 0, 0]);
            ...
            assert_eq!(output.target.capacity(), 8);
            assert_eq!(output.target.as_slice(), &[0, 0, 0, 0, 0, 0, 79, 79]);

            // Update the first 4 bytes of the reservation.
            output.write_bytes_into_reserved_exact(&mut reservation, &[1, 2, 3, 4]).unwrap();
            assert_eq!(output.target.len(), 8);
            assert_eq!(output.target.capacity(), 8);
            assert_eq!(output.target.as_slice(), &[1, 2, 3, 4, 0, 0, 79, 79]);
            assert_eq!(reservation.0.start, 4);
            assert_eq!(reservation.0.end, 6);

            // Write more stuff to the end.
            output.write_bytes_exact(&[0, 1, 0, 1]).unwrap();
            let new_capacity = output.target.capacity(); // We don't know how much it'll grow.
            assert_eq!(output.target.len(), 12);
            assert!(output.target.capacity() >= 12);
            assert_eq!(output.target.as_slice(), &[1, 2, 3, 4, 0, 0, 79, 79, 0, 1, 0, 1]);

            // Finish writing the reservation.
            output.write_bytes_into_reserved_exact(&mut reservation, &[0, 6]).unwrap();
            assert_eq!(output.target.len(), 12);
            assert_eq!(output.target.capacity(), new_capacity);
            assert_eq!(output.target.as_slice(), &[1, 2, 3, 4, 0, 6, 79, 79, 0, 1, 0, 1]);
