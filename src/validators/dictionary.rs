...
fn check_dictionary_key_type(type_ref: &TypeRef, error_reporter: &mut ErrorReporter) -> bool {
    ...
    if type_ref.is_optional {
        error_reporter.report(LogicKind::CannotUseOptionalAsKey, Some(type_ref));
        return false;
    }
    ...
    let (is_valid, named_symbol): (bool, Option<&dyn NamedSymbol>) = match definition.concrete_type() {
        Types::Struct(struct_def) => {
            ...
            if !struct_def.is_compact {
                error_reporter.report(LogicKind::StructsMustBeCompactToBeAKey, Some(type_ref));
            ...
            if contains_invalid_key_types {
                ...
                error_reporter.report(error, Some(type_ref));
                error_reporter.report(
                    ErrorKind::new_note(format!("struct '{}' is defined here:", struct_def.identifier())),
                    Some(struct_def),
                );
    ...
    if !is_valid {
        ...
        error_reporter.report(LogicKind::TypeCannotBeUsedAsAKey(pluralized_kind), Some(type_ref));
