...
fn check_dictionary_key_type(type_ref: &TypeRef, diagnostic_reporter: &mut DiagnosticReporter) -> bool {
    ...
    if type_ref.is_optional {
        diagnostic_reporter.report_error(Error::new(LogicKind::KeyMustBeNonOptional, Some(type_ref.span())));
        return false;
    }
    ...
    let (is_valid, named_symbol): (bool, Option<&dyn NamedSymbol>) = match definition.concrete_type() {
        Types::Struct(struct_def) => {
            ...
            if !struct_def.is_compact {
                let diagnostic = Error::new_with_notes(LogicKind::StructKeyMustBeCompact, Some(type_ref.span()), vec![
                    Note::new(
            ...
            for member in struct_def.members() {
                if !check_dictionary_key_type(member.data_type(), diagnostic_reporter) {
                    diagnostic_reporter.report_error(Error::new(
                        LogicKind::KeyTypeNotSupported(member.identifier().to_owned()),
                        Some(member.span()),
                    ));
            ...
            if contains_invalid_key_types {
                let diagnostic = Error::new_with_notes(
                    LogicKind::StructKeyContainsDisallowedType(struct_def.identifier().to_owned()),
                    Some(type_ref.span()),
                    vec![Note::new(
                        format!("struct '{}' is defined here:", struct_def.identifier()),
                        Some(struct_def.span()),
    ...
    if !is_valid {
        ...
        let mut diagnostic = Error::new(LogicKind::KeyTypeNotSupported(pluralized_kind), Some(type_ref.span()));
