...
lalrpop_mod!(pub lalrpop, "/parsers/slice/grammar.rs");
...
fn construct_interface(
    ...
    let bases = bases
        .unwrap_or_else(Vec::new) // Create an empty vector if no bases were specified.
        .into_iter()
...
fn construct_operation(
    ...
    let mut return_type = return_type.unwrap_or_else(Vec::new);
...
fn construct_parameter(
    ...
    (comment, attributes): (Option<DocComment>, Vec<Attribute>),
    identifier: Identifier,
    (is_streamed, tag, type_attributes): (bool, Option<u32>, Vec<Attribute>),
    mut data_type: TypeRef,
    span: Span,
) -> OwnedPtr<Parameter> {
    // Append any additional type attributes to the data type.
...
fn set_stream_modifier(
    parser: &mut Parser,
    (is_streamed, tag, attributes): (bool, Option<u32>, Vec<Attribute>),
    (start, end): (Location, Location),
) -> (bool, Option<u32>, Vec<Attribute>) {
    // The stream keyword can only be applied once. If it's already set, manually emit a syntax error.
    if is_streamed {
...
fn set_tag_value(
    parser: &mut Parser,
    (is_streamed, old_tag, attributes): (bool, Option<u32>, Vec<Attribute>),
    tag: u32,
    (start, end): (Location, Location),
) -> (bool, Option<u32>, Vec<Attribute>) {
    // A parameter can only be tagged once. If a tag is already set, manually emit a syntax error.
    if old_tag.is_some() {
        let parse_error = ParseError::UnrecognizedToken {
...
fn construct_enumerator(
    ...
    // If this is the first enumerator in the enum its implicit value is '0', otherwise it's (1 + last_value).
...
fn construct_type_alias(
    ...
    identifier: Identifier,
    type_attributes: Vec<Attribute>,
    mut underlying: TypeRef,
    span: Span,
) -> OwnedPtr<TypeAlias> {
    // Append any additional type attributes to the underlying type.
    underlying.attributes.extend(type_attributes);
...
fn construct_dictionary(
    key_attributes: Vec<Attribute>,
    mut key_type: TypeRef,
    value_attributes: Vec<Attribute>,
    mut value_type: TypeRef,
) -> OwnedPtr<Dictionary> {
    // Append any additional type attributes to the key and value types.
    key_type.attributes.extend(key_attributes);
    value_type.attributes.extend(value_attributes);
...
fn construct_attribute(raw_directive: Identifier, arguments: Option<Vec<String>>, span: Span) -> Attribute {
    let arguments = arguments.unwrap_or_else(Vec::new);
