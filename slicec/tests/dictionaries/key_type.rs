...
fn optional_keys_are_disallowed() {
    ...
    // Assert
    let expected = Diagnostic::from_error(Error::KeyMustBeNonOptional);
...
fn disallowed_primitive_types(key_type: &str) {
    ...
    // Assert
    let expected = Diagnostic::from_error(Error::KeyTypeNotSupported { kind: key_type.to_owned() });
    check_diagnostics(diagnostics, [expected]);
}
...
fn collections_are_disallowed(key_type: &str, key_kind: &str) {
    ...
    // Assert
    let expected = Diagnostic::from_error(Error::KeyTypeNotSupported { kind: key_kind.to_owned() });
    check_diagnostics(diagnostics, [expected]);
}
...
fn non_compact_structs_are_disallowed() {
    ...
    // Assert
    let expected = Diagnostic::from_error(Error::StructKeyMustBeCompact);
    check_diagnostics(diagnostics, [expected]);
}
