...
fn construct_error_from(parse_error: ParseError, file_name: &str) -> Diagnostic {
    match parse_error {
        ...
        ParseError::User {
            ...
            Diagnostic::from_error(converted).set_span(&Span::new(start, end, file_name))
        }
        ...
        ParseError::UnrecognizedToken {
            ...
            Diagnostic::from_error(Error::Syntax { message }).set_span(&Span::new(start, end, file_name))
        }

        // The parser hit EOF in the middle of a grammar rule.
        ParseError::UnrecognizedEof { location, expected } => {
            let message = generate_message(&expected, "EOF");
            Diagnostic::from_error(Error::Syntax { message }).set_span(&Span::new(location, location, file_name))
        }
